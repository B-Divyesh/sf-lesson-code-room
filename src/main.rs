...
fn api_error(error: &'static str, message: impl Into<String>) -> ApiError {
    ApiError { error, message: message.into() }
}
...
async fn main() {
    tracing_subscriber::fmt()
        .json()
        .with_env_filter(tracing_subscriber::EnvFilter::from_default_env().add_directive("lesson_code_room=info".parse().unwrap()))
        .init();

    let port: u16 = env::var("PORT").ok().and_then(|value| value.parse().ok()).unwrap_or(8080);
    let database_url = env::var("DATABASE_URL").unwrap_or_else(|_| "sqlite://data/lesson-code-room.db".to_string());
    let static_dir = env::var("STATIC_DIR").unwrap_or_else(|_| "dist".to_string());
    let billing_base = env::var("BILLING_BASE_URL").unwrap_or_else(|_| "https://api.sociobot.in".to_string());
    ...
    let db = SqlitePoolOptions::new().max_connections(8).connect(&database_url).await.expect("connect sqlite");
    sqlx::migrate!().run(&db).await.expect("run database migrations");
    let state = AppState { db, rate: Arc::new(tokio::sync::Mutex::new(HashMap::new())), billing_base };
    let app = build_app(state, &static_dir);
    let address = SocketAddr::from(([0, 0, 0, 0], port));
    let listener = tokio::net::TcpListener::bind(address).await.expect("bind server");

    info!(port, build_sha = BUILD_SHA, database_config = if env::var("DATABASE_URL").is_ok() { "supplied" } else { "generated default" }, "lesson-code-room started");
...
fn build_app(state: AppState, static_dir: &str) -> Router {
    let api = Router::new()
        ...
        .route("/rooms/{id}/progress", get(get_progress).post(update_progress))
        .route("/demo", post(create_demo))
        .route_layer(middleware::from_fn_with_state(state.clone(), rate_limit));

    let fallback = ServeDir::new(static_dir).not_found_service(ServeFile::new(format!("{static_dir}/index.html")));
    Router::new()
...
async fn create_room(State(state): State<AppState>, Json(input): Json<CreateRoom>) -> Result<Json<CreatedRoom>, ApiError> {
    ...
    Ok(Json(CreatedRoom { room, teacher_token, paid_capacity: licensed }))
}

async fn get_room(State(state): State<AppState>, AxumPath(id): AxumPath<String>) -> Result<Json<PublicRoom>, ApiError> {
    Ok(Json(find_room(&state.db, &normalize_room_id(&id)).await?))
}

async fn join_room(State(state): State<AppState>, AxumPath(id): AxumPath<String>, Json(input): Json<JoinRoom>) -> Result<Json<JoinedRoom>, ApiError> {
    let room_id = normalize_room_id(&id);
    let room = find_room(&state.db, &room_id).await?;
    let name = input.name.trim();
    if name.is_empty() || name.chars().count() > 24 {
        return Err(api_error("invalid_name", "Use a screen name from 1 to 24 characters."));
    }
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM participants WHERE room_id = ?").bind(&room_id).fetch_one(&state.db).await.map_err(db_error)?;
    if count >= room.capacity {
        return Err(api_error("room_full", "This room is full. Ask the teacher to open another room."));
    }
    let id = random_token(9);
    let learner_token = random_token(24);
    let now = unix_time();
    sqlx::query("INSERT INTO participants (id, room_id, learner_token, name, status, joined_at, updated_at) VALUES (?, ?, ?, ?, 'joined', ?, ?)")
        .bind(&id).bind(&room_id).bind(&learner_token).bind(name).bind(now).bind(now)
        .execute(&state.db).await.map_err(db_error)?;
    let participant = Participant { id, name: name.to_string(), status: "joined".to_string(), joined_at: now, updated_at: now };
    Ok(Json(JoinedRoom { participant, learner_token }))
}

async fn update_progress(State(state): State<AppState>, AxumPath(id): AxumPath<String>, Json(input): Json<ProgressUpdate>) -> Result<Json<Participant>, ApiError> {
    let room_id = normalize_room_id(&id);
    find_room(&state.db, &room_id).await?;
    if !matches!(input.status.as_str(), "ran" | "done") {
        return Err(api_error("invalid_status", "Progress must be Ran code or Done."));
    }
    let now = unix_time();
    ...
    if result.rows_affected() == 0 {
        return Err(api_error("forbidden", "This learner link is no longer active. Join the room again."));
    }
...
async fn get_progress(State(state): State<AppState>, AxumPath(id): AxumPath<String>, Query(query): Query<TeacherQuery>) -> Result<Json<ProgressResponse>, ApiError> {
    let room_id = normalize_room_id(&id);
    let expected = get_teacher_token(&state.db, &room_id).await?;
    if !constant_time_eq(expected.as_bytes(), query.teacher_token.as_bytes()) {
        return Err(api_error("forbidden", "The teacher link is not valid for this room."));
    }
    ...
    Ok(Json(ProgressResponse { participants, counts }))
}
...
async fn create_demo(State(state): State<AppState>) -> Result<Json<DemoRoom>, ApiError> {
    ...
    let teacher_token = get_teacher_token(&state.db, &room.id).await?;
    let now = unix_time();
    for (index, (name, status)) in [("Moss Finch", "done"), ("Blue Comet", "ran"), ("Quiet Fox", "joined")].iter().enumerate() {
    ...
    Ok(Json(DemoRoom { room, teacher_token }))
}

async fn insert_room(db: &SqlitePool, input: CreateRoom, licensed: bool, demo: bool) -> Result<PublicRoom, ApiError> {
    let id = unique_room_id(db).await?;
    let teacher_token = random_token(32);
    let now = unix_time();
    let ttl = if demo { DEMO_TTL_SECONDS } else { ROOM_TTL_SECONDS };
    let expires_at = now + ttl;
    let capacity = if licensed { PAID_CAPACITY } else { FREE_CAPACITY };
    ...
    Ok(PublicRoom { id, title: input.title.trim().to_string(), instructions: input.instructions.trim().to_string(), html: input.html, css: input.css, javascript: input.javascript, capacity, is_demo: demo, expires_at })
}
...
async fn find_room(db: &SqlitePool, id: &str) -> Result<PublicRoom, ApiError> {
    ...
    if room.expires_at <= unix_time() {
        return Err(api_error("expired", "This room has expired. Ask the teacher to create a new room."));
    }
    Ok(room)
}
...
async fn get_teacher_token(db: &SqlitePool, id: &str) -> Result<String, ApiError> {
    sqlx::query_scalar("SELECT teacher_token FROM rooms WHERE id = ? AND expires_at > ?")
        .bind(id).bind(unix_time()).fetch_optional(db).await.map_err(db_error)?
...
async fn unique_room_id(db: &SqlitePool) -> Result<String, ApiError> {
    ...
    for _ in 0..8 {
        ...
        let id: String = bytes.iter().map(|value| LETTERS[*value as usize % LETTERS.len()] as char).collect();
        let exists: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM rooms WHERE id = ?").bind(&id).fetch_one(db).await.map_err(db_error)?;
        if exists == 0 { return Ok(id); }
    }
    Err(api_error("server_error", "A room code could not be created. Try again."))
}

async fn verify_license(state: &AppState, token: &str) -> bool {
    let url = format!("{}/api/v1/products/lesson-code-room/verify", state.billing_base.trim_end_matches('/'));
    match reqwest::Client::new().get(url).query(&[("license", token)]).timeout(Duration::from_secs(4)).send().await {
        Ok(response) if response.status().is_success() => response.json::<LicenseVerdict>().await.map(|v| v.valid).unwrap_or(false),
        Ok(response) => { warn!(status = %response.status(), "license verification failed"); false }
        Err(error) => { warn!(%error, "license verification unavailable"); false }
    }
}

fn validate_room(input: &CreateRoom) -> Result<(), ApiError> {
    if input.title.trim().is_empty() || input.title.chars().count() > 80 {
        return Err(api_error("invalid_room", "Use an exercise title from 1 to 80 characters."));
    }
    if input.instructions.trim().is_empty() || input.instructions.chars().count() > 600 {
        return Err(api_error("invalid_room", "Add instructions from 1 to 600 characters."));
    }
    for (label, value) in [("HTML", &input.html), ("CSS", &input.css), ("JavaScript", &input.javascript)] {
        if value.len() > 50_000 { return Err(api_error("invalid_room", format!("Keep {label} under 50 KB."))); }
    }
    Ok(())
}

async fn purge_expired(db: &SqlitePool) {
    if let Err(error) = sqlx::query("DELETE FROM rooms WHERE expires_at <= ?").bind(unix_time()).execute(db).await {
        warn!(%error, "could not purge expired rooms");
    }
}

async fn rate_limit(State(state): State<AppState>, ConnectInfo(address): ConnectInfo<SocketAddr>, request: Request, next: Next) -> Response {
    let key = request.headers().get("x-forwarded-for").and_then(|value| value.to_str().ok()).and_then(|value| value.split(',').next()).map(str::trim).filter(|value| value.parse::<IpAddr>().is_ok()).map(str::to_string).unwrap_or_else(|| address.ip().to_string());
    let mut windows = state.rate.lock().await;
    let now = Instant::now();
    let window = windows.entry(key).or_insert(RateWindow { started: now, count: 0 });
...
async fn security_headers(request: Request<Body>, next: Next) -> Response {
    ...
    headers.insert("content-security-policy", HeaderValue::from_static("default-src 'self'; script-src 'self'; style-src 'self'; img-src 'self' data: blob:; font-src 'self'; connect-src 'self' https://api.sociobot.in; frame-src 'self' blob:; object-src 'none'; base-uri 'self'; form-action 'self' https://api.sociobot.in; frame-ancestors 'none'"));
    response
}
...
fn normalize_room_id(id: &str) -> String { id.trim().to_ascii_uppercase() }
fn unix_time() -> i64 { SystemTime::now().duration_since(UNIX_EPOCH).unwrap_or_default().as_secs() as i64 }
fn db_error(error: sqlx::Error) -> ApiError { warn!(%error, "database error"); api_error("server_error", "The room could not be updated. Try again.") }
